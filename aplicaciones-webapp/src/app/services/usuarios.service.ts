...
export class UsuariosService {
  ...
  public validarUsuario(data: any, editar: boolean){
    ...
    if(!this.validatorService.required(data["curp"])){
      ...
      alert("La longitud de caracteres para el CURP es menor, deben ser 18");
    }else if(!this.validatorService.max(data["curp"], 18)){
      error["curp"] = this.errorService.max(18);
      alert("La longitud de caracteres para el CURP es mayor, deben ser 18");
    }
    //RFC
    if(!this.validatorService.required(data["rfc"])){
      ...
      alert("La longitud de caracteres para el RFC es menor, deben ser 12");
    }else if(!this.validatorService.max(data["rfc"], 13)){
      error["rfc"] = this.errorService.max(13);
      alert("La longitud de caracteres para el RFC es mayor, deben ser 13");
    }
